...
def main():
    ...
    # keep only what we want (ignore post_meditation)
    KEEP = {"baseline_warmup", "meditation_all"}
    seg_df = seg_df[seg_df["segment"].isin(KEEP)].reset_index(drop=True)
    ...
    print("Example: analysis_outputs/segmented/EmotiBit_1/meditation_all/PPG.csv")
...
python3 analysis/02.1_export_segments.py \
  ...
  --segments_csv analysis_outputs/segments/session_segments.csv \
  --out analysis_outputs/segmented
"""